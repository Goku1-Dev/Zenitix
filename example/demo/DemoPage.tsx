import { h, onMount } from "../../src/index";
import {
  activeTab,
  setActiveTab,
  isSidebarOpen,
  toggleSidebar,
  theme,
  toggleTheme,
} from "./demo.state";
import "./DemoStyles.css";

import SignalsDemo from "./SignalsDemo";
import SharedDemo from "./SharedDemo";
import ListDemo from "./ListDemo";
import DerivedDemo from "./DerivedDemo";
import EffectDemo from "./EffectDemo";
import AsyncDemo from "./AsyncDemo";
import StateDemo from "./StateDemo";
import WhenDemo from "./WhenDemo";
import LayoutTest from "../LayoutTest";

const modules = [
  { id: "signals", title: "Signals",       desc: "Atomic state primitives",   component: SignalsDemo },
  { id: "list",    title: "Collections",   desc: "Reactive arrays & lists",   component: ListDemo    },
  { id: "shared",  title: "Shared State",  desc: "Cross-component sync",      component: SharedDemo  },
  { id: "derived", title: "Derived",       desc: "Auto-computed values",      component: DerivedDemo },
  { id: "effects", title: "Effects",       desc: "Side effects & lifecycle",  component: EffectDemo  },
  { id: "async",   title: "Async/Suspense",desc: "Integrated async flow",     component: AsyncDemo   },
  { id: "state",   title: "State Guard",   desc: "Mutation protection",       component: StateDemo   },
  { id: "When",   title: "When / Whenever",   desc: "Lifecycle & Conditions",       component: WhenDemo   },
  {
    id: "layout",
    title: "Layout System",
    desc: "Class-based reactivity",
    component: LayoutTest,
  },
];

export default function DemoPage() {
  onMount(() => {
    document.documentElement.setAttribute("data-theme", theme);
  });

  return (
    <div id="demo-layout">
      {/* Mobile overlay */}
      <div
        class={() => (isSidebarOpen ? "overlay active" : "overlay")}
        onClick={() => toggleSidebar()}
      />

      {/* Sidebar */}
      <div id="demo-sidebar" class={() => (isSidebarOpen ? "open" : "")}>
        <div class="sidebar-header">
          <h1>
            Zenitix.js
            <span class="demo-badge">v1.0</span>
          </h1>
          <div class="sidebar-eyebrow">Engine Explorer</div>
        </div>

        <nav class="sidebar-nav">
          {modules.map((m, i) => (
            <div
              class={() => activeTab === m.id ? "sidebar-item active" : "sidebar-item"}
              data-index={String(i + 1).padStart(2, "0")}
              onClick={() => setActiveTab(m.id)}
            >
              <span class="sidebar-item-title">{m.title}</span>
              <span class="sidebar-item-desc">{m.desc}</span>
            </div>
          ))}
        </nav>
      </div>

      {/* Main */}
      <div id="demo-main">
        <header id="demo-header">
          <div class="header-left">
            <button class="menu-toggle" onClick={toggleSidebar} aria-label="Toggle sidebar">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                <line x1="3" y1="8"  x2="21" y2="8"  />
                <line x1="3" y1="16" x2="21" y2="16" />
              </svg>
            </button>

            <div class="demo-title-container">
              <h2>{() => modules.find((m) => m.id === activeTab)?.title}</h2>
              <div class="subtitle">
                {() => modules.find((m) => m.id === activeTab)?.desc}
              </div>
            </div>
          </div>

          <div class="header-actions">
            <div class="header-links">
              <a href="https://gokulsureshkumar.in/" target="_blank" class="social-link">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <circle cx="12" cy="12" r="10" />
                  <line x1="2" y1="12" x2="22" y2="12" />
                  <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
                </svg>
                <span class="social-label">gokulsureshkumar.in</span>
              </a>
              <a href="https://github.com/Goku1-Dev" target="_blank" class="social-link">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22" />
                </svg>
                <span class="social-label">GitHub</span>
              </a>
            </div>

            <button class="theme-toggle" onClick={toggleTheme}>
              {() =>
                theme === "dark" ? (
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <circle cx="12" cy="12" r="5" />
                    <line x1="12" y1="1"     x2="12" y2="3"     />
                    <line x1="12" y1="21"    x2="12" y2="23"    />
                    <line x1="4.22" y1="4.22"  x2="5.64" y2="5.64"  />
                    <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
                    <line x1="1"  y1="12"    x2="3"  y2="12"    />
                    <line x1="21" y1="12"    x2="23" y2="12"    />
                    <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
                    <line x1="18.36" y1="5.64"  x2="19.78" y2="4.22"  />
                  </svg>
                ) : (
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
                  </svg>
                )
              }
              <span class="theme-label">
                {() => (theme === "dark" ? "Light" : "Dark")}
              </span>
            </button>
          </div>
        </header>

        <div id="demo-content-container">
          {() => modules.find((m) => m.id === activeTab)?.component()}
        </div>
      </div>
    </div>
  );
}